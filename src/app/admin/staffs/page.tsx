...
import { Users, Pencil, Plus, Trash2, Phone } from "lucide-react";
...
const StaffsPage = () => {
  ...
  const { getStaffs, deleteStaff } = useStaffActions();

  const [originalData, setOriginalData] = useState<Staff[]>([]);
  ...
  const activeCount = originalData.filter((s) => s.status).length;
  const inactiveCount = originalData.filter((s) => !s.status).length;
              ...
              <div className="flex justify-end items-center mb-2">
                <Link href="/admin/staffs/add">
                  <Button className="app-button">
                    <Plus className="mr-2" size={16} /> Add Staff
                  </Button>
                </Link>
              </div>
